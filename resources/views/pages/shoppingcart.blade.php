...
@section('content')
                ...
                <div class="mx-auto w-full">
                    <div id="shoppingCart" class="space-y-6">
                            ...
                            <div id="shopping-cart-total-container" class="space-y-6">
                                <dl
                                    ...
                                    <dd class="text-base font-bold text-gray-900 dark:text-white">
                                        Rp. <span
                                            id="shopping-cart-total">{{ number_format(array_sum(array_map(fn($item) => $item['price'] * $item['quantity'], session('cart'))), 0, ',', '.') }}</span>
                                    </dd>
                                </dl>
                        ...
                        @else
                            ...
                            <div id="empty-shopping-cart-message" class="flex items-center justify-center h-10">
                                <p class="text-gray-500 dark:text-gray-400">Keranjang kosong</p>
                            </div>
                            <div id="back-shopping" class="space-y-6">
                                <a href="{{ route('product') }}"
                                    class="flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Kembali
                                    Belanja</a>
                            </div>
                        @endif
                    </div>
    ...
    {{-- <script>
        function updateShoppingCart(cart, total, subtoal) {
            ...
            const shoppingCartTotalContainer = document.querySelector('#shopping-cart-total-container');
            const checkoutButtonContainer = document.querySelector('#view-cart-button-container');
            ...
            if (Object.keys(cart).length === 0) {
                ...
                emptyShoppingCartMessage.className = 'flex items-center justify-center h-10';
                emptyShoppingCartMessage.innerHTML = `<p class="text-gray-500 dark:text-gray-400">Keranjang kosong</p>`;
                cartDropdown.appendChild(emptyShoppingCartMessage);
                ...
                backShoppingButton.innerHTML = ` <a href="{{ route('product') }}"
                            class="flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Kembali
                            Belanja</a>`;
                cartDropdown.appendChild(backShoppingButton);
            } else {
                for (const [id, item] of Object.entries(cart)) {
                    const productShoppingCartElement = document.createElement('div');
                    productShoppingCartElement.id = `shoppingcart-item-${id}`;
                    productShoppingCartElement.className =
                        'rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6 relative';
                    productShoppingCartElement.innerHTML = ` <button onclick="removeFromShoppingCart(${id})"
                                class="absolute top-2 right-2 text-gray-500 hover:text-red-600 dark:hover:text-red-500">
                                <svg class="h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                ...
                const shoppingCartTotalContainer = document.createElement('div');
                shoppingCartTotalContainer.id = 'shopping-cart-total-container';
                shoppingCartTotalContainer.className = 'space-y-6';
                shoppingCartTotalContainer.innerHTML = `<dl
                            class="flex items-center justify-between gap-4 border-t border-gray-200 pt-2 dark:border-gray-700">
                            <dt class="text-base font-bold text-gray-900 dark:text-white">Total</dt>
                            <dd class="text-base font-bold text-gray-900 dark:text-white">
                        ...
                        <a href="#"
                            class="flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Proses
                            ke Checkout</a>`;
                shoppingCart.appendChild(shoppingCartTotalContainer);
